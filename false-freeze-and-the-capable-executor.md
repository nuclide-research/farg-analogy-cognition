...
> Note: this essay was written before the measurement in `EXPERIMENT.md`. The sweep confirmed the core claim, a fluent executor false-freezes far more than a dumb one, but corrected two things stated below. For a real model the mechanism is dilution, not the clean wall-dodging described here. And the effect saturates across frontier models rather than scaling with capability. Read this for the argument, `EXPERIMENT.md` for the data.

Swap the dumb proposer for a capable model and the elegant answer collapses. Under a byte-for-byte identical control law, the toy's plurality answer `wyz` fell from 143 of 300 runs with a dumb proposer to about one in five with a capable one. The capable model proposed clean, locally coherent readings. They scored a coherence of 0.93, cleared the default 0.90 freeze bar, and committed before the base rule was ever tried and snagged. The smarter executor reached the worse answer. It was smart enough to route around the productive failure.

That inverts the intuition. A more capable executor does not lower false-freeze risk. It raises it. A fluent model clears a low quality bar instantly and commits to the first locally coherent answer before the productive failure ever fires. The step from a dumb proposer to a fluent one raises the risk sharply. Past that step it saturates, it does not keep climbing with each increment of capability, which the sweep in `EXPERIMENT.md` confirms. The defense is not a smarter model. It is a convergence gate set in the control layer and anchored on a check the executor does not author.
